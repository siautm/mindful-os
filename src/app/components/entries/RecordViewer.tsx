...
import { EntriesHudDialog, type EntriesHudDialogState } from "./EntriesHudDialog";
import { RecordBackPanel } from "./RecordBackPanel";
...
export function RecordViewer({
  ...
  const visual = getVisualPage(visualPages);
  ...
  const discardEmptyVisual = useCallback(() => {
    setVisualPages((prev) => {
      const v = prev[0];
      if (v && !v.sourceText.trim() && !v.diagramTitle.trim()) return [];
      return prev;
    });
    setEditingVisual(false);
  }, []);
  ...
  const requestClose = useCallback(() => {
    if (isDirty && !isLocked) {
      setHudDialog({
        ...
        message: "Unsaved changes to this record will be lost.",
  ...
  const handlePickVisualType = (type: VisualType) => {
    const page = newVisualPage(type, 0);
    setVisualPages([page]);
  ...
  const diagramCenter = visual?.diagramTitle.trim() || "Untitled";
    ...
    <>
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-3">
                ...
                >
                  {cardSide === "back" ? "DIAGRAM SIDE" : `RECORD ${entry.id.slice(0, 12)}`}
                </div>
                {isDirty && !isLocked && (
              ...
              {cardSide === "front" ? (
                ...
                <h2 id="record-viewer-title" className="text-2xl font-semibold text-cyan-100 truncate pr-4">
                  {displayTitle}
                </h2>
              )}

              {cardSide === "front" && !isLocked && (
                <input
                  type="url"
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              <CardFlipButton side={cardSide} onClick={handleFlip} />
              <LockToggleButton
                isLocked={isLocked}
              ...
              {cardSide === "back" ? (
                ...
                >
                  <RecordBackPanel
                    ...
                    onPickType={handlePickVisualType}
                    onVisualChange={(next) => setVisualPages([next])}
                    onRemoveVisual={handleRemoveVisual}
                    onEdit={() => setEditingVisual(true)}
                    onDoneEdit={() => setEditingVisual(false)}
                    onZoom={() => setDiagramZoom(true)}
                  />
                </motion.div>
              ) : (
                <motion.div
      ...
      <AnimatePresence>
        {diagramZoom && visual && (
          <DiagramZoomModal
            page={visual}
            diagramTitle={diagramCenter}
            onClose={() => setDiagramZoom(false)}
          />
        )}
      </AnimatePresence>
    </>

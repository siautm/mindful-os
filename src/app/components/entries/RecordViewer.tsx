...
export function RecordViewer({
  ...
  return (
    ...
    >
      <motion.div
        className={`relative backdrop-blur-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col border ${isLocked ? "border-violet-400/50 bg-slate-900/98" : "border-cyan-500/35 bg-slate-900/95"}`}
        ...
        >
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-1 h-6 shrink-0 ${isLocked ? "bg-violet-400" : "bg-cyan-400"}`} />
              <div className={`text-[10px] font-mono tracking-widest ${isLocked ? "text-violet-600" : "text-cyan-600"}`}>
            ...
            <div className="flex items-start gap-4">
              <div className="shrink-0 space-y-2">
                {photoUrl ? (
                  ...
                  <div
                    className={`w-20 h-20 border border-dashed flex items-center justify-center ${isLocked ? "border-violet-400/40" : "border-cyan-400/40"}`}
                    style={{ clipPath: photoClip }}
                  >
                    <ImagePlus className={`w-7 h-7 ${isLocked ? "text-violet-400/50" : "text-cyan-400/50"}`} />
                  </div>
                )}
                {!isLocked && (
                  <>
          ...
          <div className="flex items-center gap-2 shrink-0 ml-2">
            <button
              ...
              className={`p-2 transition-colors ${isLocked ? "hover:bg-violet-100 text-violet-600" : "hover:bg-cyan-400/10 text-cyan-600"}`}
              style={{ clipPath: clipSm }}
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-1 h-4 ${isLocked ? "bg-violet-400" : "bg-cyan-400"}`} />
              <Tag className={`w-3.5 h-3.5 ${isLocked ? "text-violet-500" : "text-cyan-500"}`} />
              <span className={`text-[11px] font-mono tracking-widest ${isLocked ? "text-violet-400/80" : "text-slate-400"}`}>
                CLASSIFICATION TAGS
              </span>
            </div>
            ...
            <div className="space-y-2.5 mb-3">
              {metadata.map((item, idx) => (
                <div key={idx} className="flex gap-2 items-center group">
                  <div
                    className={`w-1.5 h-1.5 shrink-0 ${isLocked ? "bg-violet-400" : "bg-cyan-400"}`}
                  ...
                  <input
                    ...
                    className={`w-1/3 min-w-0 px-3 py-2 text-xs border font-mono uppercase tracking-wide ${
                      isLocked
                        ? "border-violet-400/40 bg-violet-950/30 text-slate-500 cursor-not-allowed"
                        : "border-cyan-500/30 bg-slate-950/50 focus:outline-none focus:border-cyan-400"
                    }`}
                    style={{ clipPath: clipSm }}
                  />
                  <span className={`font-mono shrink-0 ${isLocked ? "text-violet-500" : "text-cyan-400"}`}>:</span>
                  <input
                    type="text"
                    value={item.value}
                    ...
                    className={`flex-1 min-w-0 px-3 py-2 text-sm border ${
                      isLocked
                        ? "border-violet-400/30 bg-slate-950/40 text-slate-400 cursor-not-allowed"
